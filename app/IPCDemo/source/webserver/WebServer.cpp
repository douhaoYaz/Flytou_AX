...
/* http event callback */
static void SendHttpData(WSMsg* msg)
{
    HttpConn* stream = msg->conn;
    CAXRingElement* pData = (CAXRingElement*)msg->packet;
    delete msg;
    msg = nullptr;
...
static CVideoEncoder* GetVenc(AX_U32 nInnerIndex)
{
    for (const auto& venc : g_vecVEnc) {
        if (nInnerIndex == venc->m_nInnerIndex) {
            return venc;
        }
    }
...
static void CameraAction(HttpConn* conn)
{
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        ...
        AX_U8 nNoneBias = ADAPTER_BOOLSTR2INT(mprGetJson(jsonCamera, "camera_attr.nonebias"));
        if (nNoneBias != attr.nNoneBias) {
            attr.nNoneBias = nNoneBias;
            pStageOption->SetCamera(attr);
            AX_BOOL bNoneBias = (0 == nNoneBias) ? AX_FALSE : AX_TRUE;
            gOptions.SetLoadNoneBiasBinEnable(bNoneBias);
            bRestartISP = AX_TRUE;
        }
        ...
        if (nRotation != nOldRotation
            || nMirror != nOldMirror
            || nFlip != nOldFlip) {
            /* Stop web app preview before resolution changed
               to avoid preview blurred for resolution changed by multi-webpage */
            ...
            /* Restart web app preview after resolution change done */
            if (nRotation != nOldRotation) {
                g_webserver.RestartPreview();
            }
        }
...
static void VideoAction(HttpConn* conn)
{
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        AX_CHAR szAttr[128] = {0};
        for (const auto& item : g_tEPOptions) {
            if (item.eEPType == E_END_POINT_VENC) {
                string strKey = "video";
                strKey += to_string(item.nInnerIndex);
                if (pStageOption->GetVideoRcSetInfoStr(item.nChannel, szAttr, 128)) {
                    mprWriteJsonObj(mprGetJsonObj(pResponseBody, PARAM_KEY_DATA),
                                    strKey.c_str(),
                                    mprParseJson(szAttr));
                }
            }
        ...
        LOG_M(WEB, "recv:%s", mprJsonToString(jsonVideo, MPR_JSON_QUOTES));

        for (const auto& item : g_tEPOptions) {
            if (item.eEPType == E_END_POINT_VENC) {
                string strVideoKey = "video";
                strVideoKey += to_string(item.nInnerIndex);
                MprJson* _jsonObj = mprReadJsonObj(jsonVideo, strVideoKey.c_str());
                if (!_jsonObj) {
                    LOG_M_E(WEB, "mprReadJsonObj for %s failed!", strVideoKey.c_str());
                    continue;
                }

                AX_U8  nMaxIProp = atoi(mprGetJson(_jsonObj, "max_iprop"));
                AX_U32 nRcType   = atoi(mprGetJson(_jsonObj, "rc_type"));
                VIDEO_RC_SET_INFO_T tRcSetInfo = pStageOption->GetVideoRcSetInfo(item.nChannel);
                if ((nRcType == (AX_U32)tRcSetInfo.eRCType) && nMaxIProp == tRcSetInfo.nMaxIProp) {
                    continue;
                }
                CVideoEncoder* pVenc = GetVenc(item.nInnerIndex);
                AX_BOOL bRcTypeChanged = (nRcType == (AX_U32)tRcSetInfo.eRCType) ? AX_FALSE : AX_TRUE;

                if (pVenc && pVenc->UpdateRCParam(nRcType, bRcTypeChanged, nMaxIProp)) {
                    tRcSetInfo.eRCType = (RcType)nRcType;
                    tRcSetInfo.nMaxIProp = nMaxIProp;
                    pStageOption->SetVideoRcSetInfo(item.nChannel, tRcSetInfo);
                }

                if (bRcTypeChanged) {
                    /* Restart web app preview after RcType change done */
                    g_webserver.RestartPreview();
                }
            }
        }
...
static void AiAction(HttpConn* conn)
{
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        ...
        tNewAiAttr.tEvents.tSCD.nEnable     = ADAPTER_BOOLSTR2INT(mprGetJson(jsonAI, "ai_attr.events.scene_change_detect.enable"));
        tNewAiAttr.tEvents.tSCD.nThrsHoldY  = atoi(mprGetJson(jsonAI, "ai_attr.events.scene_change_detect.threshold_y"));
        tNewAiAttr.tEvents.tSCD.nConfidence  = atoi(mprGetJson(jsonAI, "ai_attr.events.scene_change_detect.confidence"));
        ...
        CSCD::GetInstance()->SetThreshold(tNewAiAttr.tEvents.tSCD.nThrsHoldY, tNewAiAttr.tEvents.tSCD.nConfidence);
        ...
        gOptions.ActiveSceneChangeDetect(tNewAiAttr.tEvents.tSCD.nEnable ? AX_TRUE : AX_FALSE);
    }
...
static void WebNotifier(HttpConn *conn, int event, int arg)
{
    ...
    if (bNeedClose) {
        int nIndex = mprRemoveItem(g_pClients, conn);
        if (nIndex != -1) {
            g_webserver.UpdateConnStatus();
        }
    }
}
...
static void WSPreviewAction(HttpConn* conn)
{
    ...
    if (nullptr != szToken && strlen(szToken) > 0) {
        ...
        LOG_M(WEB, "[%d] preview connected: %p", nChnID, conn);
    }
...
AX_VOID CWebServer::SendWSData()
{
    ...
    for (AX_S32 next = 0; (client = (HttpConn *)MprListGetNextItem(g_pClients, &next))!= 0;) {
        ...
        nChannel = pWebServer->GetWSChn(client);
        if (nChannel == -1 || nChannel >= MAX_WS_CONN_NUM) {
            continue;
        }
        ...
        gPrintHelper.Add(E_PH_MOD_WEB_CONN, 0, 0);
        {
            ...
            if (!pWebServer->m_arrConnStatus[nChannel]) {
...
AX_VOID CWebServer::SendEventsData(WEB_EVENTS_DATA_T* data)
{
    if (!m_bServerStarted) {
        return;
    }
...
std::string CWebServer::FormatEventsJson(WEB_EVENTS_DATA_T* pEvent)
{
    ...
    if (E_WEB_EVENTS_TYPE_MD == pEvent->eType) {
        ...
        snprintf(szEventJson, MAX_EVENTS_CHN_SIZE, "{\"events\": [{\"type\": %d, \"date\": \"%s\", \"info\": \"%s\"}]}", pEvent->eType, szDate, pEvent->tSCD.szDisplay);
    } else if (E_WEB_EVENTS_TYPE_ReStartPreview == pEvent->eType) {
        snprintf(szEventJson, MAX_EVENTS_CHN_SIZE, "{\"events\": [{\"type\": %d, \"date\": \"%s\"}]}", pEvent->eType, szDate);
    }
...
AX_S32 CWebServer::GetWSChn(void* conn)
{
    if (!conn) {
        return -1;
    }
    ...
    if (pWSData) {
        std::lock_guard<std::mutex> guard(m_mtxWSData);
...
AX_VOID CWebServer::UpdateConnStatus()
{
    AX_S32 nChannel = -1;
