...
AX_S32 CFileUtils::MkDir(const AX_CHAR* pszDir)
{
    DIR *mydir = NULL;
...
AX_S32 CFileUtils::CheckAndMkDir(const AX_CHAR* pszDir)
{
    AX_S32  nRet  = 0;
    const AX_CHAR *pDir = pszDir;

    while (1) {
        const AX_CHAR *pIndex = index(pDir, '/');
