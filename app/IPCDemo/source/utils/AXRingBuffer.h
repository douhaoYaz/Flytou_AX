...
public:
    ...
    AX_BOOL Put(CAXRingElement &element) {
        ...
        if ((m_nTail - m_nHeader) == m_nElementCount) {
            // is full
            LOG_M_I(AXRING, "[%s] is full, tail=%llu, head=%llu", m_szName, m_nTail, m_nHeader);
            if (element.bIFrame) {
                ...
                if (nRefCount == 0) {
                    m_nTail--;
                    m_bHasLost = AX_FALSE;
                    LOG_M_I(AXRING, "[%s] ---, replace last one with i frame for is full", m_szName);
                } else {
                    m_bHasLost = AX_TRUE;
                    LOG_M_I(AXRING, "[%s] ---, drop one i frame for is full", m_szName);
                    return AX_FALSE;
                }
                ...
                LOG_M_I(AXRING, "[%s] ---, drop one p frame for is full", m_szName);
                return AX_FALSE;
            }
            ...
            if (element.bIFrame)  {
                ...
                if (m_bHasLost)  {
                    // drop this P Frame
                    LOG_M_I(AXRING, "[%s] ---, drop one p frame for lost yet", m_szName);
                    return AX_FALSE;
                }
            }
        }
        ...
        if (nRefCount != 0) {
            m_bHasLost = AX_TRUE;
            LOG_M_I(AXRING, "[%s] ---, drop one frame for refcount is none zero, nIndex=%llu, nRefCount=%d", m_szName, nIndex, nRefCount);
            return AX_FALSE;
        }
