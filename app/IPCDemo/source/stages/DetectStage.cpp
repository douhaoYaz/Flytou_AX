...
AX_BOOL CDetectStage::ProcessFrame(CMediaFrame* pFrame)
{
    ...
    if (!bIvesFrameSkip) {
        ...
        if (!m_bReseting) {
            ...
            if (gOptions.IsActivedSceneChangeDetect()) {
                CSCD::GetInstance()->ProcessFrame(pFrame);
            }
        }
        m_mtxReset.unlock();
    }
    ...
    if (!bAiFrameSkip) {
        if (gOptions.IsActivedDetect() && gOptions.IsActivedDetectFromWeb()) {
            ...
            if (CDetector::GetInstance()->ProcessFrame(pFrame, nAIDetectGaps)) {
...
AX_BOOL CDetectStage::Init()
{
    ...
    // if (gOptions.IsActivedSceneChangeDetect()) {
    {
        if (!CSCD::GetInstance()->Startup(nFrmW, nFrmH)) {
            return AX_FALSE;
        }
    }
...
AX_BOOL CDetectStage::Reset(const DETECT_STAGE_INFO_T &tInfo)
{
    ...
    // if (gOptions.IsActivedMotionDetect()) {
    {
     ...
     CSCD::GetInstance()->Update(tInfo.nFrmWidth, tInfo.nFrmHeight);
