  ...
  auto describe_data_type = [](AX_JOINT_DATA_TYPE_T type) -> const char* {
    switch (type) {
      ...
      case AX_JOINT_DATA_TYPE_T::AX_JOINT_DT_FLOAT64:
        return "float64";
      case AX_JOINT_DATA_TYPE_T::AX_JOINT_DT_SINT64:
        return "sint64";
      case AX_JOINT_DATA_TYPE_T::AX_JOINT_DT_UINT64:
        return "uint64";
...
#elif defined(CHIP_AX620) || defined(CHIP_AX720)
...
enum CacheMode {
  CM_CACHED = 0,        // 表示只使用带cache的CMM内存
  CM_NONE_CACHED = 1,   // 表示只使用不带cache的CMM内存
  CM_SMART_CACHED = 2,  // 表示分别使用带cache和不带cache的CMM内存
};

CacheMode get_cache_mode(std::string cache_mode) {
  std::transform(cache_mode.begin(), cache_mode.end(), cache_mode.begin(),
                 ::toupper);
  if (cache_mode == "CACHED") {
    return CM_CACHED;
  } else if (cache_mode == "NONE-CACHED") {
    return CM_NONE_CACHED;
  } else if (cache_mode == "SMART-CACHED") {
    return CM_SMART_CACHED;
  } else {
    printf("unsupported cache mode %s\n", cache_mode.c_str());
    assert(0);
  }
}

std::string get_cache_mode_text(CacheMode mode) {
  switch (mode) {
    case CM_CACHED:
      return "CACHED";
    case CM_NONE_CACHED:
      return "NONE-CACHED";
    case CM_SMART_CACHED:
      return "SMART-CACHED";
    default:
      printf("unsupported cache mode %d\n", mode);
      assert(0);
  }
}
...
\t'file*' would be directly loaded in binary format to tensor in order",
  ...
  args.add<std::string>(
      "cache-mode", '\0',
      "'CACHED' means use only cached CMM memory; 'NONE-CACHED' means use only "
      "none-cached CMM memory; 'SMART_CACHED' means use cached and "
      "none-cached CMM memory in turn",
      false, "CACHED");
  ...
  CacheMode use_cache_mode =
      get_cache_mode(args.get<std::string>("cache-mode"));
  const int max_experimental_group =
      (use_cache_mode == CM_SMART_CACHED) ? 2 : 1;
  std::string output_info[max_experimental_group];
  int high_performance_group = 0;
  int min_duration = INT_MAX;
  AX_BOOL no_cache_mem[max_experimental_group];
  CacheMode cache_mode[max_experimental_group];
  if (use_cache_mode == CM_CACHED) {
    no_cache_mem[0] = AX_FALSE;
    cache_mode[0] = CM_CACHED;
  } else if (use_cache_mode == CM_NONE_CACHED) {
    no_cache_mem[0] = AX_TRUE;
    cache_mode[0] = CM_NONE_CACHED;
  } else if (use_cache_mode == CM_SMART_CACHED) {
    no_cache_mem[0] = AX_FALSE;
    no_cache_mem[1] = AX_TRUE;
    cache_mode[0] = CM_CACHED;
    cache_mode[1] = CM_NONE_CACHED;
  }

  for (int group = 0; group < max_experimental_group; ++group) {
    ...
    ctx_settings.bNoCacheMem = no_cache_mem[group];
    assert(AX_ERR_NPU_JOINT_SUCCESS ==
    ...
    if (cmm_ret != 0) {
      output_info[group] += "Get CMM usage failed\n\n";
    } else {
      char char_buf[256];
      sprintf(char_buf, "CMM usage: %u\n\n", cmm.nCMMSize);
      output_info[group] += char_buf;
    }
    ...
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
                        time_end - time_start) /
                    test_count;
    if ((int)duration.count() < min_duration) {
      min_duration = (int)duration.count();
      high_performance_group = group;
    }
    ...
    {
      char char_buf[256];
      sprintf(char_buf, "Run task took %d us (%d rounds for average)\n",
    ...
    if (has_neu_comp) {
      char char_buf[256];
      sprintf(char_buf, "\tRun NEU took an average of %d us (overhead %d us)\n",
    ...
    if (has_onnx_comp) {
      char char_buf[256];
      sprintf(char_buf,
              "\tRun ONNX took an average of %d us (overhead %d us)\n",
    ...
    if (has_axe_comp) {
      char char_buf[256];
      sprintf(char_buf, "\tRun AXE took an average of %d us (overhead %d us)\n",
    ...
    {
      char char_buf[256];
      sprintf(char_buf, "NPU perf group total: %ld\n", work_cycle.total_cycle);
      output_info[group] += char_buf;
    }
#else
    {
      char char_buf[256];
      sprintf(char_buf, "NPU perf group total: %lld\n", work_cycle.total_cycle);
      output_info[group] += char_buf;
    }
#endif
#if defined(CHIP_AX630A)
    ...
    {
      char char_buf[256];
      sprintf(char_buf, "\tNPU perf group of eu(%d): %d\n", k,
              work_cycle.eu_cycle[k]);
      output_info[group] += char_buf;
    }
    ...
    if (use_cache_mode != max_experimental_group)
      break;  // 只有 SMART-CACHED 模式才会跑2组
  }

  printf("use cache mode: %s\n\n",
         get_cache_mode_text(cache_mode[high_performance_group]).c_str());
  printf("%s\n", output_info[high_performance_group].c_str());
