...
static AX_S32 gBufWidth = 1920;
static AX_S32 gBufHeight = 1088;
static AX_S32 gBufCnt = 10;
...
AX_VIDEO_FRAME_INFO_S tUserPic[AX_VDEC_MAX_GRP_SIZE];
...
void *H264DecFrameFunc(void *arg)
{
    ...
    tVdecAttr.u32PicHeight = 1088;  /*Max pic height*/
    tVdecAttr.u32StreamBufSize = 8 * 1024 * 1024;
    tVdecAttr.u32FrameBufCnt = gBufCnt;
    ...
    FrameSize = AX_VDEC_GetPicBufferSize(gBufWidth, gBufHeight, PT_H264); //3655712;
    ...
    do {
        ...
        if ((sRet == AX_ERR_VDEC_NOBUF) || (sRet == AX_ERR_VDEC_ILLEGAL_PARAM) || (sRet == AX_ERR_VDEC_NOT_SUPPORT)) {
            printf("Decoder SendStream error sRet: %x\n", sRet);
            /*init video stream info*/
            tStrInfo.pu8Addr = NULL;
            tStrInfo.u32Len = 0x0;  /*stream len*/
            tStrInfo.bEndOfFrame = AX_TRUE;
...
void *VdecThread(void *arg)
{
    ...
    FrameSize = AX_VDEC_GetPicBufferSize(gBufWidth, gBufHeight, PT_JPEG); //3655712;
    printf("Get pool mem size is %d\n", FrameSize);
...
static void PrintHelp()
{
    ...
    printf("  -W:       output buffer width.        (for pool GetPicBufferSize), default: 1920\n");
    printf("  -H:       outbut buffer height.       (for pool GetPicBufferSize), default: 1088\n");
    printf("  -b:       output buffer number.       (for Frame Buffer Cnt), default: 10\n");
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "c:L:t:w:m:W:H:b:M:T:u:h")) != -1) {
        isExit = 0;
        switch (c) {
        case 'c':
        ...
        case 'W':
            gBufWidth = atoi(optarg);
            break;
        case 'H':
            gBufHeight = atoi(optarg);
            break;
        case 'b':
            gBufCnt = atoi(optarg);
            break;
    ...
    if (userPicTest) {
        for (i = 0; i < gChnNumber; i++) {
            ...
            s32Ret = PoolUserPicInit(&tUserPic[i]);
            if (AX_SUCCESS != s32Ret) {
                printf("PoolUserPicInit Failed!\n");
                return -1;
            }

            s32Ret = PoolUserPicDisplay(&tUserPic[i]);
            if (AX_SUCCESS != s32Ret) {
                printf("PoolUserPicInit Failed!\n");
                return -1;
            }
    ...
    if (gGrpAttr.enType == PT_JPEG) {
        ...
        for (i = 0; i < gChnNumber; i++) {
            ...
            GrpArgs[i].pstUsrPic = &tUserPic[i];
