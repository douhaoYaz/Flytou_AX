...
#define OS04A10_SDR_620U_NO_PFR_BIN_FILE_NAME "/opt/etc/os04a10_sdr_620u_no_pfr_0.bin"
#define OS04A10_HDR_620U_NO_PFR_BIN_FILE_NAME "/opt/etc/os04a10_hdr_620u_no_pfr_0.bin"
#define OS04A10_SDR_620A_NO_PFR_BIN_FILE_NAME "/opt/etc/os04a10_sdr_620a_no_pfr_0.bin"
#define OS04A10_HDR_620A_NO_PFR_BIN_FILE_NAME "/opt/etc/os04a10_hdr_620a_no_pfr_0.bin"
...
#ifdef SAMPLE_IRCUT
AX_U32 ax_float_convert_to_int(float value, unsigned int int_bit, unsigned int frac_bit, bool signed_value)
{
...
void *IspIrRun(void *args)
{
    AX_S32 preDayNightMode = -1;
    AX_U32 dayNightMode = 0;
    AX_S32 axRet = 0;
    AX_ISP_IQ_IR_PARAM_T ispIrParam = {0};

    axRet = AX_VIN_GetChnDayNightMode(gCams[0].nPipeId, 0, (AX_DAYNIGHT_MODE_E *)&dayNightMode);
    if (0 != axRet) {
        COMM_ISP_PRT("AX_VIN_GetChnDayNightMode failed, ret=0x%x.\n", axRet);
        return NULL;
    }

    ispIrParam.nIrCalibR                = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nIrCalibR, 8, 10, false);
    ispIrParam.nIrCalibG                = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nIrCalibG, 8, 10, false);
    ispIrParam.nIrCalibB                = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nIrCalibB, 8, 10, false);
    ispIrParam.nNight2DayIrStrengthTh   = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nNight2DayIrStrengthTh, 8, 10, false);
    ispIrParam.nNight2DayIrDetectTh     = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nNight2DayIrDetectTh, 8, 10, false);
    ispIrParam.nDay2NightLuxTh          = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nDay2NightLuxTh, 22, 10, false);
    ispIrParam.nNight2DayLuxTh          = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nNight2DayLuxTh, 22, 10, false);
    ispIrParam.nNight2DayBrightTh       = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nNight2DayBrightTh, 8, 10, false);
    ispIrParam.nNight2DayDarkTh         = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nNight2DayDarkTh, 8, 10, false);
    ispIrParam.nNight2DayUsefullWpRatio = ax_float_convert_to_int(gCams[0].stSnsIrAttr.nNight2DayUsefullWpRatio, 8, 10, false);
    ispIrParam.nCacheTime = 2;
    ispIrParam.nInitDayNightMode = dayNightMode;
    preDayNightMode = dayNightMode;

    COMM_ISP_PRT("Get Ir Param == %f %f %f %f %f %f %f %f %f %f || %d %d %d %d %d %d %d %d %d %d\n", gCams[0].stSnsIrAttr.nIrCalibR, gCams[0].stSnsIrAttr.nIrCalibG, gCams[0].stSnsIrAttr.nIrCalibB,
                gCams[0].stSnsIrAttr.nNight2DayIrStrengthTh, gCams[0].stSnsIrAttr.nNight2DayIrDetectTh, gCams[0].stSnsIrAttr.nDay2NightLuxTh, gCams[0].stSnsIrAttr.nNight2DayLuxTh,
                gCams[0].stSnsIrAttr.nNight2DayBrightTh, gCams[0].stSnsIrAttr.nNight2DayDarkTh, gCams[0].stSnsIrAttr.nNight2DayUsefullWpRatio,
                ispIrParam.nIrCalibR, ispIrParam.nIrCalibG, ispIrParam.nIrCalibB, ispIrParam.nNight2DayIrStrengthTh, ispIrParam.nNight2DayIrDetectTh,
                ispIrParam.nDay2NightLuxTh, ispIrParam.nNight2DayLuxTh, ispIrParam.nNight2DayBrightTh, ispIrParam.nNight2DayDarkTh, ispIrParam.nNight2DayUsefullWpRatio);

    if (ispIrParam.nInitDayNightMode == AX_DAYNIGHT_MODE_NIGHT) {
        axRet = AX_VIN_SetChnDayNightMode(gCams[0].nPipeId, 0, AX_DAYNIGHT_MODE_NIGHT);
        if (axRet != 0) {
            COMM_ISP_PRT("AX_VIN_SetChnDayNightMode failed, ret=0x%x.\n", axRet);
            return NULL;
        }
        // TO DO IRCUT
    } else if (ispIrParam.nInitDayNightMode == AX_DAYNIGHT_MODE_DAY) {
        axRet = AX_VIN_SetChnDayNightMode(gCams[0].nPipeId, 0, AX_DAYNIGHT_MODE_DAY);
        if (axRet != 0) {
            COMM_ISP_PRT("AX_VIN_SetChnDayNightMode failed, ret=0x%x.\n", axRet);
            return NULL;
        }
    ...
    axRet = AX_ISP_IQ_SetIrParam(gCams[0].nPipeId, &ispIrParam);
    if (axRet != 0) {
        COMM_ISP_PRT("AX_VIN_GetChnDayNightMode failed, ret=0x%x.\n", axRet);
        return NULL;
    }
    ...
    while (!g_isp_force_loop_exit) {
        axRet = AX_ISP_IQ_GetDayNightStatus(gCams[0].nPipeId, &dayNightMode);
        if (axRet != 0) {
            COMM_ISP_PRT("AX_VIN_SetChnDayNightMode failed, ret=0x%x.\n", axRet);
        }

        if (dayNightMode != preDayNightMode) {
            switch(dayNightMode) {
                case 0:
                    /* night to day */
                    axRet = AX_VIN_SetChnDayNightMode(gCams[0].nPipeId, 0, AX_DAYNIGHT_MODE_DAY);
                ...
                case 1:
                    ...
                    break;
                default:
                    COMM_ISP_PRT("Unkonw dayNightMode [%d]\n", dayNightMode);
                    break;
            }
        }
...
#ifdef SAMPLE_IRCUT
    pthread_create(&gCams[0].tIspIrThread, NULL, IspIrRun, NULL);
#endif
...
#ifdef SAMPLE_IRCUT
    axRet = pthread_join(gCams[0].tIspIrThread, NULL);
    if (axRet < 0) {
...
int main(int argc, char *argv[])
{
    ...
    if (eSysCase == SYS_CASE_SINGLE_OS04A10) {
        ...
        #ifdef SAMPLE_IRCUT
        COMMON_ISP_GetIrCutAttr(OMNIVISION_OS04A10, &gCams[0].stSnsIrAttr);
        #endif
        ...
        if (eHdrMode == AX_SNS_LINEAR_MODE) {
            snprintf(gCams[0].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_SDR_620U_NO_PFR_BIN_FILE_NAME);
            snprintf(gCams[1].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_SDR_620U_NO_PFR_BIN_FILE_NAME);

        } else if (eHdrMode == AX_SNS_HDR_2X_MODE) {
            snprintf(gCams[0].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_HDR_620U_NO_PFR_BIN_FILE_NAME);
            snprintf(gCams[1].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_HDR_620U_NO_PFR_BIN_FILE_NAME);
        }
#else
        if (eHdrMode == AX_SNS_LINEAR_MODE) {
            snprintf(gCams[0].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_SDR_620A_NO_PFR_BIN_FILE_NAME);
            snprintf(gCams[1].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_SDR_620A_NO_PFR_BIN_FILE_NAME);
        } else if (eHdrMode == AX_SNS_HDR_2X_MODE) {
            snprintf(gCams[0].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_HDR_620A_NO_PFR_BIN_FILE_NAME);
            snprintf(gCams[1].szTuningFileName, MAX_FILE_NAME_CHAR_SIZE, "%s", OS04A10_HDR_620A_NO_PFR_BIN_FILE_NAME);
        }
#endif
