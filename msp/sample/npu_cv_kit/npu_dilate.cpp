...
int main(int argc, char* argv[]) {
    ...
    AX_NPU_CV_Dilate_Context context = nullptr;

    AX_NPU_CV_DilateParam param;
    std::vector<AX_U8> mat33 = {0,255,0,255,255,255,0,255,0};
    memcpy(param.nKernelValue, mat33.data(), sizeof(param));

    auto time_start = std::chrono::system_clock::now();
    for (int i = 0; i < nRepeat; ++i) {
        assert(0 == AX_NPU_CV_Dilate(&context, virtual_npu_mode_type, &a_matrix_2d, &result_matrix_2d, &param));
        if(nRepeat > 1 && i == 0){
    ...
    AX_NPU_CV_DestroyDilateContext(context);
    printf("\n**************************************************\n");
    ...
    printf("**************************************************\n");
     ...
     if(!output.empty()){
        ...
        if (stat(output.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) {
            ...
            char fname[256];
            sprintf(fname, "%s/npu_dilate.row_%d.column_%d.npu_mode_%d.%s",
                output.c_str(),
                nRow, nColumn,
    ...
    if (bCheck) {
        ...
        if (match_gt) {
            printf("The dilate result match with GT!\n");
        } else {
            printf("The dilate result mismatch with GT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        }
