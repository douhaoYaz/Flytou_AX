...
int main(int argc, char* argv[]) {
    ...
    args.add<std::string> ("data-type", '\0', "int8", false, "int8");
    ...
    // LD_LIBRARY_PATH=./lib:$LD_LIBRARY_PATH ./bin/npu_abs.ax620a  --mode=disable --mode-type=disable --data-type=float --row 128 --column 128 --repeat=100 --check=1
    ...
    if(data_type != AX_NPU_CV_DT_INT8){
    ...
    AX_NPU_CV_DataType result_data_type = AX_NPU_CV_DT_UINT8;
    ...
    for (int i = 0; i < nRepeat; ++i) {
        assert(0 == AX_NPU_CV_ABS(virtual_npu_mode_type, &a_matrix_2d, &result_matrix_2d));
        if(nRepeat > 1 && i == 0){
    ...
    printf("**************************************************\n");
     ...
     if(!output.empty()){
        ...
        if (stat(output.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) {
            ...
            char fname[256];
            sprintf(fname, "%s/npu_abs.row_%d.column_%d.npu_mode_%d.%s",
                output.c_str(),
                nRow, nColumn,
    ...
    if (bCheck) {
        ...
        // Calculate GT
        AX_U8* gt_data = new AX_U8[nRow * nColumn];
        memset(gt_data, 0, nRow * nColumn * sizeof(AX_U8));

        AX_S8 *a_data = (AX_S8 *)a_matrix_2d.pVir;
        printf("gt data---------------------->\n");
        for (int r = 0; r < nRow; ++r) {
            for (int c = 0; c < nColumn; ++c) {
                gt_data[r*nColumn + c] = std::abs(a_data[r*nColumn + c]);
                if (c % 16 == 0) printf("\n");
                printf("%d ", gt_data[r*nColumn + c]);
            }
            printf("\n");
        }
        ...
        printf("dump abs results:\n");
        AX_U8* result_data = (AX_U8*)result_matrix_2d.pVir;
        for (int r = 0; r < nRow; ++r) {
            for (int c = 0; c < nColumn; ++c) {
                if (c % 16 == 0) printf("\n");
                printf("%d ", result_data[r * nColumn + c]);
            }
            printf("\n");
        }
        ...
        if (match_gt) {
            printf("The abs result match with GT!\n");
        } else {
            printf("The abs result mismatch with GT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        }
