...
int main(int argc, char* argv[]) {
    ...
    // LD_LIBRARY_PATH=./lib:$LD_LIBRARY_PATH ./bin/npu_arctan2.ax620a  --mode=disable --mode-type=disable --data-type=float --row 128 --column 128 --repeat=100 --check=1
    ...
    AX_NPU_CV_Matrix2D a_matrix_2d = create_matrix_universal(matrix_a, nRow, nColumn, data_type, -100, 100);
    AX_NPU_CV_Matrix2D b_matrix_2d = create_matrix_universal(matrix_b, nRow, nColumn, data_type, 1, 100);
    ...
    for (int i = 0; i < nRepeat; ++i) {
        assert(0 == AX_NPU_CV_Arctan2(virtual_npu_mode_type, &a_matrix_2d, &b_matrix_2d, &result_matrix_2d));
        if(nRepeat > 1 && i == 0){
    ...
    printf("**************************************************\n");
     ...
     if(!output.empty()){
        ...
        if (stat(output.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) {
            ...
            char fname[256];
            sprintf(fname, "%s/npu_arctan2.row_%d.column_%d.%s.npu_mode_%d.%s",
                output.c_str(),
                nRow, nColumn,
    ...
    if (bCheck) {
        ...
        printf("src2 data===================================\n");
        print_matrix(b_matrix_2d);

        AX_NPU_CV_Matrix2D tang_mat = create_matrix_by_size(nRow, nColumn, AX_NPU_CV_DT_FLOAT32);
        for(int i = 0; i < nRow * nColumn; i++){
            float a, b;
            switch(data_type){
                case AX_NPU_CV_DT_INT8:
                {
                    AX_S8 * pa = (AX_S8*)a_matrix_2d.pVir;
                    AX_S8 * pb = (AX_S8*)b_matrix_2d.pVir;
                    a = (float)(pa[i]);
                    b = (float)(pb[i]);
                    break;
                }
                case AX_NPU_CV_DT_UINT8:
                {
                    AX_U8 * pa = (AX_U8*)a_matrix_2d.pVir;
                    AX_U8 * pb = (AX_U8*)b_matrix_2d.pVir;
                    a = (float)(pa[i]);
                    b = (float)(pb[i]);
                    break;
                }
                case AX_NPU_CV_DT_FLOAT32:
                {
                    AX_F32 * pa = (AX_F32*)a_matrix_2d.pVir;
                    AX_F32 * pb = (AX_F32*)b_matrix_2d.pVir;
                    a = (float)(pa[i]);
                    b = (float)(pb[i]);
                    break;
                }
            ...
            float * tang_data = (float*)tang_mat.pVir;
            if(b == 0) b = 1e-8;
            tang_data[i] = a/b;
        }

        matrix_map(tang_mat, gt_mat, std::atan);
        printf("**************************************************\n");
        printf("gt data====================================\n");
        print_matrix(gt_mat);

        printf("dump arctan2 results:\n");
        print_matrix(result_matrix_2d);

        // Check GT with result
        bool match_gt = matrix_cmp(result_matrix_2d, gt_mat);
        if (match_gt) {
            printf("The arctan2 result match with GT!\n");
        } else {
            printf("The arctan2 result mismatch with GT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        }

        release_matrix_memory(gt_mat);
        release_matrix_memory(tang_mat);
    }
