...
AX_S32 SAMPLE_IVES_SCD_ENTRY(const AX_CHAR *pRefImg, const AX_CHAR *pCurImg, AX_U32 nWidth, AX_U32 nHeight) {
    ...
    AX_IVES_IMAGE_S *pstImgs[2] = {NULL};
    AX_SCD_CHN_ATTR_S stChnAttr = {.chn = 1, .stArea = {0, 0, nWidth, nHeight}, .u8Thrd = 60, .u8Confidence = 60};

    for (i = 0; i < 2; ++i) {
        pstImgs[i] = SAMPLE_LOAD_IMAGE(IMAGE_FILE_PATH[i], nWidth, nHeight, AX_YUV420_SEMIPLANAR);
        if (!pstImgs[i]) {
            goto EXIT0;
        }
