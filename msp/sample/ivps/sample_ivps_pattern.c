...
{
    AX_U32 u32Value : 24;
} __attribute__((__packed__)) COLOR_RGB24_T;
...
typedef struct UTIL_COLOR_COMPONENT
{
    AX_U32 u32Length;
    AX_U32 u32Offset;
} UTIL_COLOR_COMPONENT_T;

typedef struct UTIL_RGB_INFO
{
    UTIL_COLOR_COMPONENT_T stRed;
    UTIL_COLOR_COMPONENT_T stGreen;
    UTIL_COLOR_COMPONENT_T stBlue;
    UTIL_COLOR_COMPONENT_T stAlpha;
} UTIL_RGB_INFO_T;

/* This function takes 8-bit color values */
static inline AX_U32 SHIFT_COLOR8(const UTIL_COLOR_COMPONENT_T *pstComp,
                                  AX_U32 u32Value)
{
    u32Value &= 0xff;
...
static AX_VOID FillSmpteYUV(AX_U32 u32Width, AX_U32 u32Height, AX_U32 u32Stride,
                            AX_U8 *u8YMem, AX_U8 *u8UMem, AX_U8 *u8VMem)
{
    const COLOR_YUV_T stCororsTop[] = {
    ...
    const COLOR_YUV_T stCororsMiddle[] = {
        MAKE_YUV_601(0, 0, 192),     /* blue */
    ...
    const COLOR_YUV_T stCororsBottom[] = {
...
static AX_VOID FillSmpteRGB16(const UTIL_RGB_INFO_T *stRGB, AX_VOID *pMem,
...
static AX_VOID FillSmpteRGB24(const UTIL_RGB_INFO_T *stRGB, AX_VOID *pMem,
                              AX_U32 u32Width, AX_U32 u32Height,
                              AX_U32 u32Stride)
{
    const COLOR_RGB24_T stColorsTop[] = {
    ...
    const COLOR_RGB24_T stColorsMiddle[] = {
        MAKE_RGB24(stRGB, 0, 0, 192),     /* blue */
    ...
    const COLOR_RGB24_T stColorsBottom[] = {
    ...
    for (; u32Y < u32Height; ++u32Y)
    {
        ...
        for (; u32X < u32Width * 6 / 7; ++u32X)
            ((COLOR_RGB24_T *)pMem)[u32X] =
                stColorsBottom[(u32X - u32Width * 5 / 7) * 3 / (u32Width / 7) + 4];
        for (; u32X < u32Width; ++u32X)
            ((COLOR_RGB24_T *)pMem)[u32X] = stColorsBottom[7];
        pMem += u32Stride;
    }
}
...
{
    UTIL_RGB_INFO_T stAR12_Info = MAKE_RGB_INFO(4, 8, 4, 4, 4, 0, 4, 12);
    UTIL_RGB_INFO_T stAR15_Info = MAKE_RGB_INFO(5, 10, 5, 5, 5, 0, 1, 15);
    UTIL_RGB_INFO_T stRG16_Info = MAKE_RGB_INFO(5, 11, 6, 5, 5, 0, 0, 0);
    UTIL_RGB_INFO_T stRG24_Info = MAKE_RGB_INFO(8, 16, 8, 8, 8, 0, 0, 0);
    UTIL_RGB_INFO_T stAR24_Info = MAKE_RGB_INFO(8, 16, 8, 8, 8, 0, 8, 24);
    switch (enPixFmt)
    {
