...
AX_BOOL LoadImageExt(AX_POOL PoolId, const char *pszImge, AX_U32 pImgSize, AX_BLK *BlkId, AX_BOOL isYUVSplit, AX_U64 *nPhyAddr, AX_VOID **pVirAddr)
{
    ...
    if (fp)
    {
        ...
        if (isYUVSplit)
        {
            ...
            if (0 != ret)
            {
                ALOGE("BufPoolBlockAddrGet Y fail, ret=0x%x", ret);
                fclose(fp);
                return AX_FALSE;
            }
...
AX_S32 TestPatternAutoGen(AX_POOL PoolId, AX_IMG_FORMAT_E eImgFormat, AX_U16 nStride, AX_U32 nWidth, AX_U32 nHeight,
                          AX_BLK *BlkId, AX_U64 *nPhyAddr, AX_VOID **pVirAddr)
{
    ...
    PatternSmpteFill(eImgFormat, nStride, nWidth, nHeight, *pVirAddr);
    return 0;
}
