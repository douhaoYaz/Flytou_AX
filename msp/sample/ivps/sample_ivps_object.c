    ...
    while (!bThreadLoopExit)
    {
        ...
        for (i = 0; i < pThis->tPipelineAttr.nOutChnNum; i++)
        {
            if (!pThis->arrOutChns[i].bEmpty)
            {
                ...
                printf("Get IVPS frame: Chn: %d, %d x %d stride: %d, phy: %llx, userdata: %llx, seq_num: %lld, pts: %llx, blockid: %x framesize:%d\n", i,
                       ...
                       tDstFrame[i].stVFrame.u32FrameSize);
...
AX_S32 ImageBuf_Get(AX_VIDEO_FRAME_INFO_S *ptImage, char *pImgFile)
{
    ...
    if (pImgFile)
    {
        ...
        {
            ALOGE("FILE:%s load fail!", pImgFile);
            return -1;
        }
