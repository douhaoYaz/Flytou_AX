...
AX_S32 main(AX_S32 argc, char *argv[])
{
    ...
    arrBlkInfo[0].nCnt = 3;
    arrBlkInfo[1].nSize = CalcImgSize(3840, 3840, 2160, AX_YUV420_SEMIPLANAR, 16);
    arrBlkInfo[1].nCnt = 3;
    ...
    if (ret)
    {
        ALOGE("IVPS_PoolFloorInit failed, ret=0x%x.\n", ret);
        goto error0;
    }
    ...
    if (access(pFrameFile, 0))
    {
        ...
        ret = TestPatternAutoGen(-1, tFrame.stVFrame.enImgFormat, tFrame.stVFrame.u32PicStride[0],
                                 tFrame.stVFrame.s16OffsetRight - tFrame.stVFrame.s16OffsetLeft,
                                 tFrame.stVFrame.s16OffsetBottom - tFrame.stVFrame.s16OffsetTop,
                                 &tFrame.stVFrame.u32BlkId[0], &tFrame.stVFrame.u64PhyAddr[0],
                                 (AX_VOID **)&tFrame.stVFrame.u64VirAddr[0]);
        if (0 != ret)
        {
    ...
    /* Output width and height is specified by user. */
    /*
    AX_U32 WidthTemp, HeightTemp;
    WidthTemp = tDstFrame.u32Width = 1280;
    HeightTemp = tDstFrame.u32Height = 720;
    tDstFrame.u32PicStride[0] = 1280;
    */
    tDstFrame.enImgFormat = AX_YUV420_SEMIPLANAR;

    if (tDstFrame.u32Width && tDstFrame.u32Height)
        nImgSize = CalcImgSize(tDstFrame.u32PicStride[0], tDstFrame.u32Width, tDstFrame.u32Height, tDstFrame.enImgFormat, 16);
    else
        nImgSize = CalcImgSize(tDstFrame.u32PicStride[0], tFrame.stVFrame.u32Width, tFrame.stVFrame.u32Height, tDstFrame.enImgFormat, 16);
    CHECK_RESULT(BufPoolBlockAddrGet(tFrame.u32PoolId, nImgSize, &tDstFrame.u64PhyAddr[0], (AX_VOID **)(&tDstFrame.u64VirAddr[0]), &BlkId));
   ...
   */
    printf("OFFSET left:%d right:%d top:%d bottom:%d\n", tDstFrame.s16OffsetLeft, tDstFrame.s16OffsetRight,
           tDstFrame.s16OffsetTop, tDstFrame.s16OffsetBottom);
