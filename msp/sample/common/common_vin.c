...
{
    if (!pSnsIrAttr) {
        COMM_PRT("Param pSnsIrAttr failed.\n");
        return -1;
    }
    ...
    switch(eSnsType) {
        ...
        case OMNIVISION_OS04A10_MASTER:
        case OMNIVISION_OS04A10_SLAVE:
            memcpy(pSnsIrAttr, &gOs04a10SnsIrAttr, sizeof(AX_SNS_IRCUT_ATTR_T));
            break;
        default:
            memcpy(pSnsIrAttr, &gOs04a10SnsIrAttr, sizeof(AX_SNS_IRCUT_ATTR_T));
            break;
...
AX_S32 COMMON_ISP_ResetSns(AX_U8 pipe, SAMPLE_SNS_TYPE_E eSnsType)
{
    ...
    if (NULL != ptSnsHdl->pfn_sensor_reset) {
        axRet = ptSnsHdl->pfn_sensor_reset(pipe);
        if (0 != axRet) {
            COMM_PRT("sensor reset failed with %#x!\n", axRet);
            return axRet;
        }
    } else {
        return -1;
