...
static AX_VOID FaceAttrResult(const AX_SKEL_OBJECT_ITEM_S *pstObjectItems, ATTRINFO_T *pAttrInfo) {
    picojson::value obj;
    ...
    for (size_t i = 0; i < pstObjectItems->nMetaInfoSize; i++) {
        if (!strcmp(pstObjectItems->pstMetaInfo[i].pstrType, "face_attr")) {
            ...
            pAttrInfo->tFaceInfo.nAge = face_attr.nAge;
            pAttrInfo->tFaceInfo.nGender = face_attr.nGender;
            strncpy(pAttrInfo->tFaceInfo.szMask, (const AX_CHAR *)face_attr.strRespirator.c_str(), sizeof(pAttrInfo->tFaceInfo.szMask) - 1);
            strncpy(pAttrInfo->tFaceInfo.szGender, (const AX_CHAR *)strGender.c_str(), sizeof(pAttrInfo->tFaceInfo.szGender) - 1);
        }
    }
}

static AX_VOID PlateAttrResult(const AX_SKEL_OBJECT_ITEM_S *pstObjectItems, ATTRINFO_T *pAttrInfo) {
    picojson::value obj;
...
static AX_VOID VehicleAttrResult(const AX_SKEL_OBJECT_ITEM_S *pstObjectItems, ATTRINFO_T *pAttrInfo) {
    picojson::value obj;
    AI_Plate_Attr_t plat_attr;
...
AX_VOID AttrParser(const AX_SKEL_OBJECT_ITEM_S *pstObjectItems, ATTRINFO_T *pAttrInfo) {
