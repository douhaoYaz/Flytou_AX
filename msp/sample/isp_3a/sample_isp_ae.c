...
AX_S32 sample_ae_sns_set_lcg_hcg(AX_U8 pipe, AX_U32 lcgHcg)
{
    AX_S32 result = 0;

    if (AX_NULL != pSnsHandle[pipe]->pfn_sensor_hcglcg_ctrl) {
        result = pSnsHandle[pipe]->pfn_sensor_hcglcg_ctrl(pipe, lcgHcg);
        if (SAMPLE_SUCCESS != result) {
            return SAMPLE_FAILED;
        }
    } else {
...
)
{
    ...
    /* driver shutter: fixed point, u22.10
     * shutter in ae algo: float , uint: us
     * Convert shutter(int, uint us) to lineNum and send to driver ( fixed point: U22.10)*/

    /* When calc lineNum, find the closest integration time.
     * If it is looking down to find the integration time,
     * in dark environment, it will not reach the maximum integration time due to UI accuracy(U32).
     */

    pCtxManual->fIntegrationTime = max(pCtxStatus->snsShutter[AX_HDR_CHN_L], pCtxConfig->IntegrationTimeLimit.fMin);
    AX_U32 lineNum = (AX_U32)(pCtxManual->fIntegrationTime / pCtxConfig->linePeriodFloat);
    AX_F32 lineNum_frac = pCtxManual->fIntegrationTime / pCtxConfig->linePeriodFloat - (AX_F32)lineNum;
    if( lineNum_frac >= 0.5){
        lineNum += 1;
    }
    pCtxManual->fIntegrationTime = lineNum * pCtxConfig->linePeriodFloat;

    // Set Sensor Exposure
    if (pCtxConfig->eSnsMode == AX_SNS_LINEAR_MODE) {

        // Set LCG / HCG
        sample_ae_sns_set_lcg_hcg(pipe, pCtxManual->uHcg_lcg);
        ...
        tShutterTbl.nIntTime[AXAE_LONG_FRAME] = lineNum * pCtxConfig->linePeriodFixedPoint;
        ...
        tShutterTbl.nIntTime[AXAE_LONG_FRAME] = lineNum * pCtxConfig->linePeriodFixedPoint;
        tShutterTbl.nHdrRatio[AXAE_LONG_FRAME] = pAlgCtx->tConfig.hdrRatio;
        tShutterTbl.nIntTime[AXAE_MEDIUM_FRAME] = lineNum * pCtxConfig->linePeriodFixedPoint;
        ...
        tShutterTbl.nIntTime[AXAE_MEDIUM_FRAME] = lineNum * pCtxConfig->linePeriodFixedPoint;
        tShutterTbl.nHdrRatio[AXAE_MEDIUM_FRAME] = pAlgCtx->tConfig.hdrRatio;
        tShutterTbl.nIntTime[AXAE_SHORT_FRAME] = lineNum * pCtxConfig->linePeriodFixedPoint;
...
)
{
    ...
    AX_F32 snsShutter      = 0.0f;  // Shutter (Sensor Integration Time, Unit:us)
    ...
    snsShutter = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nCurIntegrationTime[AXAE_LONG_FRAME], 22, 10, false);
    ...
    /* for fixed frame rate mode ,get hdrHwMaxShutter from driver(fixed point U22.10) */
    pAlgCtx->tStatus.hdrHwMaxShutter = sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMaxIntegrationTime[AXAE_LONG_FRAME], 22, 10, false);
    ...
    if (hdrMode == AX_SNS_LINEAR_MODE) {
        ...
        shortShutter  = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nCurIntegrationTime[AXAE_MEDIUM_FRAME], 22, 10, false);
        ...
        vsShutter  = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nCurIntegrationTime[AXAE_SHORT_FRAME], 22, 10, false);
...
(
    ...
    pCtxConfig->antiFlickerParam.enable = 1;
    pCtxConfig->antiFlickerParam.flickerPeriod = AX_ISP_FLICKER_100HZ;
    ...
    //Init Hardware limit.
    pCtxConfig->IntegrationTimeLimit.fMax =  sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMaxIntegrationTime[AXAE_LONG_FRAME], 22, 10, false);
    pCtxConfig->IntegrationTimeLimit.fMin =  sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMinIntegrationTime[AXAE_LONG_FRAME], 22, 10, false);
    pCtxConfig->IntegrationTimeLimit.fStep = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nIntegrationTimeIncrement[AXAE_LONG_FRAME], 22, 10, false);
    ...
    pCtxConfig->sIntegrationTime.fMax =  sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMaxIntegrationTime[AXAE_MEDIUM_FRAME], 22, 10, false);
    pCtxConfig->sIntegrationTime.fMin =  sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMinIntegrationTime[AXAE_MEDIUM_FRAME], 22, 10, false);
    pCtxConfig->sIntegrationTime.fStep = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nIntegrationTimeIncrement[AXAE_MEDIUM_FRAME], 22, 10, false);
    ...
    pCtxConfig->sVsintegrationTime.fMax =  sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMaxIntegrationTime[AXAE_SHORT_FRAME], 22, 10, false);
    pCtxConfig->sVsintegrationTime.fMin =  sample_ae_int_convert_to_float(tSnsParam.sns_ae_limit.nMinIntegrationTime[AXAE_SHORT_FRAME], 22, 10, false);
    pCtxConfig->sVsintegrationTime.fStep = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nIntegrationTimeIncrement[AXAE_SHORT_FRAME], 22, 10, false);
    ...
    pCtxConfig->linePeriodFixedPoint = tSnsParam.sns_ae_param.nIntegrationTimeIncrement[AXAE_LONG_FRAME];
    pCtxConfig->linePeriodFloat = sample_ae_int_convert_to_float(tSnsParam.sns_ae_param.nIntegrationTimeIncrement[AXAE_LONG_FRAME], 22, 10, false);
    ...
    //Max Min exposure
    pCtxConfig->fExpValLimitMax = pCtxConfig->IntegrationTimeLimit.fMax *
                                  pCtxConfig->AgainLimit.fMax * pCtxConfig->DgainLimit.fMax *
                                  pCtxConfig->IspDgainLimit.fMax * pCtxConfig->fHcglcgratio;
    pCtxConfig->fExpValLimitMin = pCtxConfig->IntegrationTimeLimit.fMin *
...
{
    pAeResult->nIntTime[AX_HDR_CHN_L]  = (AX_U32)pStatus->snsShutter[AX_HDR_CHN_L];
    ...
    pAeResult->nIntTime[AX_HDR_CHN_M]  = (AX_U32)pStatus->snsShutter[AX_HDR_CHN_M];
    ...
    pAeResult->nIntTime[AX_HDR_CHN_S]  = (AX_U32)pStatus->snsShutter[AX_HDR_CHN_S];
    ...
    pAeResult->nIntTime[AX_HDR_CHN_VS] = (AX_U32)pStatus->snsShutter[AX_HDR_CHN_VS];
    ...
    if (nHdrMode == 1){
        printf("AE Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f \n",
            pStatus->snsShutter[AX_HDR_CHN_L],  pStatus->snsTotalAgain[AX_HDR_CHN_L],
            pStatus->ispGain, pStatus->lux, pStatus->meanLuma);
    }
    else if(nHdrMode == 2){
        printf("AE Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f , realhdrRealRatioLtoS = %0.2f\n",
            pStatus->snsShutter[AX_HDR_CHN_L],  pStatus->snsTotalAgain[AX_HDR_CHN_L],
            pStatus->ispGain, pStatus->lux, pStatus->meanLuma,pStatus->hdrRealRatioLtoS);
    }
    else if(nHdrMode == 3){
        printf("AE Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f , realhdrRealRatioLtoS = %0.2f, realhdrRealRatioStoVS = %0.2f,\n",
            pStatus->snsShutter[AX_HDR_CHN_L],  pStatus->snsTotalAgain[AX_HDR_CHN_L],
...
AX_S32 sample_ae_init
(
    AX_U8 pipe,
    AX_ISP_AE_INITATTR_T *pParam,
    AX_ISP_AE_RESULT_T *pAeResult
)
{
    AX_S32 result = 0;
    ...
    for(int i = 0 ;i<tSnsParam.sns_dev_attr.eSnsMode;i++){
        ...
        tSnsParam.sns_ae_limit.nMinIntegrationTime[i] = sample_ae_float_convert_to_int(20, 22, 10, false);
        tSnsParam.sns_ae_limit.nMaxIntegrationTime[i] = sample_ae_float_convert_to_int(33000, 22, 10, false);
        tSnsParam.sns_ae_param.nCurAGain[i] = 2.0f;
        tSnsParam.sns_ae_param.nCurDGain[i] = 1.0f;
        tSnsParam.sns_ae_param.nCurIspDGain[i] = 1.0f;
        tSnsParam.sns_ae_param.nCurIntegrationTime[i] = sample_ae_float_convert_to_int(10000, 22, 10, false);
        ...
        tSnsParam.sns_ae_param.nIntegrationTimeIncrement[i] = sample_ae_float_convert_to_int(10, 22, 10, false);

    }

    tSnsParam.sns_ae_limit.nMinratio = 5.0f;
    tSnsParam.sns_ae_limit.nMaxratio = 16.0f;
    tSnsParam.sns_ae_limit.nInitMinIntegrationTime = sample_ae_float_convert_to_int(20, 22, 10, false);
    tSnsParam.sns_ae_limit.nInitMaxIntegrationTime = sample_ae_float_convert_to_int(33000, 22, 10, false);
...
)
{
    ...
    /* Convert AE algo (float) to lineNum */
    /* When calc lineNum, find the closest integration time.
     * If it is looking down to find the integration time,
     * in dark environment, it will not reach the maximum integration time due to UI accuracy(U32).
     */
    AX_U32 lineNum = (AX_U32)(SplitIntegrationTime / pCtxConfig->linePeriodFloat);
    AX_F32 lineNum_frac = SplitIntegrationTime / pCtxConfig->linePeriodFloat - (AX_F32)lineNum;
    if( lineNum_frac >= 0.5){
        lineNum += 1;
    }
    ...
    if (pCtxConfig->eSnsMode == AX_SNS_LINEAR_MODE) {
        // Set LCG / HCG
        sample_ae_sns_set_lcg_hcg(pipe, HcgLcg_Mode);
...
)
{
    ...
    AX_F32 splitIntegrationTime = 0.0f;    // For Step4
    ...
    AX_F32 algoMaxShutter = pCtxStatus->algoMaxShutter;
