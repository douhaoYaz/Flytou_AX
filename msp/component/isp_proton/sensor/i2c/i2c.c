    ...
    const AX_U8 NrOfDataBytes)
{
    ...
    default: {
        printf("%s: Wrong amount of bytes (%d) for swapping.\n", __func__, NrOfDataBytes);
        break;
    }
    }
    ...
    AX_BOOL bSwapBytesEnable)
{
    ...
    //printf("%s: addr:0x%x, fd:%d\n", __func__, reg_addr, handle);
    memset(msgs, 0x0, sizeof(struct i2c_msg) * 2);

    if ((reg_addr_size > 4) || ((p_data == NULL) && (num_data != 0))) {
    ...
    AX_BOOL bSwapBytesEnable)
{
    ...
    ret = ioctl(handle, I2C_RDWR, &msgset);
    if (ret < 0) {
        printf("%s: Failed to write reg: %s.!\n", __func__, strerror(errno));
    }
