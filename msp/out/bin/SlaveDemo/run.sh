...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup ./${process} $json_path 2 $LogLevel $ChnDbgLevel $PrintFPS $TestMode $GetYuvFrmTimeout $EnableIVPS &
else
  ./${process} $json_path $LogTarget $LogLevel $ChnDbgLevel $PrintFPS $TestMode $GetYuvFrmTimeout $EnableIVPS
fi
