...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup ./${process} $json_path 2 $LogLevel $RTSPMaxFrmSize $WebFrmSizeRatio $ActiveDetect $PrintFPS $UseWebDeamon $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize 1 0 $EnableOSD $DetectConfigPath $ActiveOD $ActiveMD $EnableAutoSleep $AutoSleepFrameNum $EnableEIS $EISDelayNum $EISCropW $EISCropH $EISEffectComp $RotationSet $MirrorSet $FlipSet $HotBalanceTest $SnsHotNoiseBalanceTest $LensDrivePath &
else
