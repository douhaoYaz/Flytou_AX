...
if [ $# == 0 ] ; then
  json_path=$cur_path/config/cascade_8_4.json
elif [ $# == 1 ] ; then
  json_path=$1
else
  echo "USAGE: $0 <configure file path>"
  echo " e.g.: $0 ./config/cascade_8_4.json"
  exit 1;
fi
...
if [ $Mp4Recorder == 1 ] ; then
  ...
  if [ $Mp4Saved2SDCard == 1 ] ; then
    umount $Mp4SavedPath
    mount /dev/mmcblk1 $Mp4SavedPath
    if [ 0 != $? ] ; then
      mount /dev/mmcblk1p1 $Mp4SavedPath
      if [ 0 != $? ] ; then
        echo "ERROR: Mount SD card failed, mp4 recorder can not work!"
      else
        echo "Mp4 saved path: \"$Mp4SavedPath\"(SDCard)"
      fi
    else
        echo "Mp4 saved path: \"$Mp4SavedPath\"(SDCard)"
    fi
...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup gdb --args ./${process} $json_path 2 $LogLevel $ChnDbgLevel $PrintFPS $UseWebDeamon $TestMode $LinkMode $ChangeVEnc1080P $VEncFpsControl $EnableHDMI $RTSPMaxFrmSize $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize $GetSnsFrmTimeout &
else
  gdb --args ./${process} $json_path $LogTarget $LogLevel $ChnDbgLevel $PrintFPS $UseWebDeamon $TestMode $LinkMode $ChangeVEnc1080P $VEncFpsControl $EnableHDMI $RTSPMaxFrmSize $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize $GetSnsFrmTimeout
fi
