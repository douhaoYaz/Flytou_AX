...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup ./${process} $json_path 2 $LogLevel $ChnDbgLevel $PrintFPS $UseWebDeamon $TestMode $LinkMode $ChangeVEnc1080P $VEncFpsControl $EnableHDMI $RTSPMaxFrmSize $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize $GetSnsFrmTimeout &
else
  ./${process} $json_path $LogTarget $LogLevel $ChnDbgLevel $PrintFPS $UseWebDeamon $TestMode $LinkMode $ChangeVEnc1080P $VEncFpsControl $EnableHDMI $RTSPMaxFrmSize $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize $GetSnsFrmTimeout
fi
