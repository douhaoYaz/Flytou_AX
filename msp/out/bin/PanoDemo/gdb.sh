...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup gdb --args ./${process} $json_path 2 $LogLevel $PrintFPS $UseWebDeamon $PanoMode $TestMode $ChangeVEnc1080P $VEncFpsControl $RTSPMaxFrmSize $WebFrmSizeRatio $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize &
else
  gdb --args ./${process} $json_path $LogTarget $LogLevel $PrintFPS $UseWebDeamon $PanoMode $TestMode $ChangeVEnc1080P $VEncFpsControl $RTSPMaxFrmSize $WebFrmSizeRatio $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize
fi
