...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup ./${process} $json_path 2 $LogLevel $RTSPMaxFrmSize $WebFrmSizeRatio $ActiveDetect $PrintFPS $UseWebDeamon $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize 1 0 $EnableNpuTest $EnableGrpOverlay $FBCChannels $DetectConfigPath $ActiveOD $EnableLDC &
else
  ./${process} $json_path $LogTarget $LogLevel $RTSPMaxFrmSize $WebFrmSizeRatio $ActiveDetect $PrintFPS $UseWebDeamon $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize 1 0 $EnableNpuTest $EnableGrpOverlay $FBCChannels $DetectConfigPath $ActiveOD $EnableLDC
fi
