...
if [ $# == 0 ] ; then
  json_path=$cur_path/config/os04a10_config.json
elif [ $# == 1 ] ; then
  json_path=$1
else
  echo "USAGE: $0 <configure file path>"
  echo " e.g.: $0 ./config/os04a10_config.json"
  exit 1;
fi
...
if [ $RunInBackground == 1 ] ; then
  ...
  nohup ./${process} $json_path 2 $LogLevel $RTSPMaxFrmSize $WebFrmSizeRatio $ActiveDetect $PrintFPS $UseWebDeamon $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize 1 $EnableDewarp $EnableGrpOverlay $DetectConfigPath $ActiveOD $ActiveMD $EnableAutoSleep $AutoSleepFrameNum $RotationSet $MirrorSet $FlipSet $HotBalanceTest &
else
  ./${process} $json_path $LogTarget $LogLevel $RTSPMaxFrmSize $WebFrmSizeRatio $ActiveDetect $PrintFPS $UseWebDeamon $Mp4Recorder $Mp4SavedPath $Mp4MaxFileNum $Mp4MaxFileSize 1 $EnableDewarp $EnableGrpOverlay $DetectConfigPath $ActiveOD $ActiveMD $EnableAutoSleep $AutoSleepFrameNum $RotationSet $MirrorSet $FlipSet $HotBalanceTest
fi
